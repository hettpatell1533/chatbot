...
export class TokenService {
    ...
    async createToken(userId: string, tokenType: TokenType, role: string, exp: number) {
        try{
            const secret = tokenType === TokenType.ACCESS_TOKEN ? this.configService.get<string>('ACCESS_TOKEN_SECRET') : tokenType === TokenType.REFRESH_TOKEN ? this.configService.get<string>('REFRESH_TOKEN_SECRET') : tokenType === TokenType.VERIFY_TOKEN ? this.configService.get<string>('VERIFY_TOKEN_SECRET') : this.configService.get<string>('FORGOT_PASSWORD_TOKEN_SECRET');
    ...
    async newAuthTokenFromRefreshToken(refreshToken: string) {
        try {
            const decoded = await this.verifyToken(refreshToken, this.configService.get<string>('REFRESH_TOKEN_SECRET')!);
